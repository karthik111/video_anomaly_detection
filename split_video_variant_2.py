# Variant 2 of split_video where
# begin_1 - spans till start of the first anomalous segment
# anomaly_1 - from start of the first anomalous segment till the video end or start of the second anomalous segment
#  anomaly_2 if there are more than 1 anomalous segment in the video
# anomaly_2 - start of the second anomalous segment till the end of the video
...
folder_path = "test_videos_variant_2"
    ...
    for index, row in df_test_times.iterrows():
        ...
        for s in samples:
            if s[0].name == row[0] and row[1] != 'Normal':
                ...
                # Variant 2: all frames till end of the first anomaly
                if b_2 == -1:
                    frames_a = vr.get_batch(range(b_1, len(vr))).asnumpy()
                else:
                    frames_a = vr.get_batch(range(b_1, b_2)).asnumpy()
                ...
                if b_2 != -1:
                    ...
                    frames_a = vr.get_batch(range(b_2, len(vr))).asnumpy()
...
def main():
    print("Calling split videos: Variant 2...")
    split_videos()
