    ...
    for i in range(n):
        ...
        if not is_processed(video_name, video_class):
            try:
                ...
                load_VIT = Load_VIT()
                features = load_VIT.extract_features(video)
                print("Extracted features: ", type(features), features['vit'].shape)
                #import numpy as np
                #features = np.mean(features[None], axis=0)
                #return feature
                #features = r3d_18_feature_extraction.extract_penultimate_features(input_vid)
                save_file(features['vit'], video_name, video_class)
